...
class BallTrainer:
    ...
    def train(self):
        ...
        for i in range(iter_num, self.max_iter_num):
            ...
            T, Tr = make_translation_batch(batch_size=BATCH_SIZE * DT_BATCH_MULTIPLE, dim=np.array([1, 0, 0]),
            ...
            # Tr = -T
            z0_rnn = self.model.predict_with_symmetry(z_gt, I_sample_points, lambda z: z)
            vae_loss = self.calc_vae_loss(data, z_gt, mu, logvar, is_log * i)
            rnn_loss = self.calc_rnn_loss(data[:, 1:, :, :, :], z_gt, z0_rnn, is_log * i, None)

            R_loss = (torch.zeros(2))
            Z_loss = (torch.zeros(2))
            T_loss = self.batch_symm_loss(
                data[:, 1:, :, :, :], z_gt, z0_rnn, T_sample_points, DT_BATCH_MULTIPLE,
    ...
    def batch_symm_loss(self, x1, z_gt, z0_rnn, sample_points, symm_batch_multiple, symm_func, symm_reverse_func):
        ...
        zloss_S_rnn_Sr_D__x1, zloss_S_rnn_Sr__z1 = \
            self.calc_symm_loss(x1_repeat, z_gt_repeat, z0_rnn_repeat, z0_S_rnn, symm_reverse_func)
    ...
    def calc_symm_loss(self, x1, z_gt, z0_rnn, z0_S_rnn, symm_reverse_func):
